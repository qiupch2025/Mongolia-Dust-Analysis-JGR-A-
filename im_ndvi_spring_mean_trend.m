...
% 区域平均趋势
ndvi_mean_series = nan(n_years, 1);
for y = 1:n_years
    tmp = ndvi_spring(:, :, y);
    ndvi_mean_series(y) = mean(tmp(inPoly), 'omitnan');
end
[b_mean, ~, ~, ~, stats_mean] = regress(ndvi_mean_series, [ones(n_years, 1), years']);
...
% 创建 NetCDF 文件
outputFile = 'NDVI_spring_trend_Inner_Mongolia.nc';
...
ncwrite(outputFile, 'ndvi_spring_mean_series', ndvi_mean_series);
% 写入数据
